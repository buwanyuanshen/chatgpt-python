...
default_settings = {
    ...
    "max_tokens": 3000,
    "continuous_chat": 0,
    "api_base": "https://api.openai-proxy.com/v1",  # 添加代理网址配置项
}
...
def toggle_simplified():
    ...
    if simplified_state:
        ...
        api_base_entry.grid(row=3, column=1, padx=(0, 10), sticky="ew")
        show_api_base_check.grid(row=4, columnspan=2, pady=(5, 0), sticky="w")
        api_key_label.grid(row=5, column=0, sticky="w")
        api_key_entry.grid(row=5, column=1, padx=(0, 10), sticky="ew")
        show_api_key_check.grid(row=6, columnspan=2, pady=(5, 0), sticky="w")
        temperature_label.grid(row=7, column=0, sticky="w")
        temperature_entry.grid(row=7, column=1, padx=(0, 10), sticky="ew")
        max_tokens_label.grid(row=8, column=0, sticky="w")
        max_tokens_entry.grid(row=8, column=1, padx=(0, 10), sticky="ew")
        continuous_chat_check.grid(row=9, columnspan=2, pady=(5, 0), sticky="w")
        user_input_label.grid(row=10, column=0, sticky="w")
        user_input_text.grid(row=10, column=1, padx=(0, 5), sticky="ew")
        send_button.grid(row=11, column=0, padx=(20, 10), pady=(10, 0), sticky="w")
        clear_button.grid(row=11, column=1, padx=(10, 20), pady=(10, 0), sticky="n")
...
def get_response_thread():
    ...
    api_base = api_base_entry.get().strip()  # 获取代理网址输入
...
# 创建tkinter窗口
root = tk.Tk()
root.title("FREE ChatGPT-----Stream-----MADE-----BY-----中北锋哥-----2024.2.25")
...
api_base_label = ttk.Label(frame_left, text="API 代理网址(填到v1)：")  # 新增代理网址配置项
api_base_label.grid(row=3, column=0, sticky="w")

api_base_entry = ttk.Entry(frame_left, width=50,show="*", font=("Arial", 11))
api_base_entry.insert(0, api_base)
api_base_entry.grid(row=3, column=1, padx=(0, 5), sticky="ew")
...
api_key_entry.grid(row=5, column=1, padx=(0, 5), sticky="ew")
...
temperature_entry.grid(row=7, column=1, padx=(0, 5), sticky="ew")
...
max_tokens_entry.grid(row=8, column=1, padx=(0, 5), sticky="ew")
...
user_copy_button.grid(row=12, column=0, padx=(20, 10), pady=(5, 0), sticky="w")

assistant_copy_button = ttk.Button(frame_left, text="复制GPT消息", command=copy_assistant_message, width=15)
assistant_copy_button.grid(row=12, column=1, padx=(10, 20), pady=(5, 0), sticky="n")
...
play_button1.grid(row=13, column=0, padx=(20, 10), pady=(5, 0), sticky="w")
...
play_button2.grid(row=13, column=1, padx=(10, 20), pady=(5, 0), sticky="n")
